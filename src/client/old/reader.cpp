...
void reader_t::handle_loop()
{
    for (;;) {
        ...
        ssize_t header_size
            = read(m_conn_fd, &header, sizeof(prot::PayloadHeader));
        ...
        if (static_cast<size_t>(header_size) < sizeof(prot::PayloadHeader)) {
            log.warn(
        ...
        log.debug("payload size is {} bytes", header.payload_size);

        util::ByteVector payload { header.payload_size };

        ssize_t actual_size
            = read(m_conn_fd, payload.data(), header.payload_size);
        ...
        if (static_cast<size_t>(actual_size) < header.payload_size) {
            log.warn(
        ...
        try {
            ...
            log.warn("handling payload failed, reason: {}", err.what());
...
void reader_t::dtor()
{
    ...
    if (::shutdown(m_conn_fd, SHUT_WR) == -1) {
        if (errno == ENOTCONN) {
            log.warn("connection shutdown failed, reason: {}", strerror(errno));
        } else {
            log.error(
...
void reader_t::handle_payload(util::ByteVector& payload)
{
    ...
    if (std::holds_alternative<prot::TaggedCommand>(payload_object)) {
        auto& tagged_command = std::get<prot::TaggedCommand>(payload_object);
    ...
    if (std::holds_alternative<prot::TaggedDrawList>(payload_object)) {
        auto& tagged_draw_list = std::get<prot::TaggedDrawList>(payload_object);
...
void reader_t::update_list(prot::TaggedCommand& tagged_command)
{
    threading::mutex_guard guard { share::writer_mutex };
    {
        threading::rwlock_wrguard wrguard { share::rwlock1 };

        common::draw_list_wrapper { share::list1 }.update(tagged_command);
    }

    {
        threading::rwlock_wrguard wrguard { share::rwlock2 };

        common::draw_list_wrapper { share::list2 }.update(tagged_command);
    }
}
